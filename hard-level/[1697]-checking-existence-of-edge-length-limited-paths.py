...
# --------------- Runtime 1937 ms, beats 75.47%. Memory 61.7MB, beats 7.55% ---------------
...
class Solution:
    def distanceLimitedPathsExist(self, n: int, edgeList: List[List[int]], queries: List[List[int]]) -> List[bool]:
        ...
        edgeList = sorted((d, u, v) for u, v, d in edgeList)

        result = [False] * len(queries)

        index = 0
        for l, p, q, i in queries:
            while index < len(edgeList) and edgeList[index][0] < l:
                _, u, v = edgeList[index]
                uf.union(u, v)
                index += 1
