...
# --------------- Runtime 77 ms, beats 95.13%. Memory 16.6MB, beats 91.99% ---------------
...
class Solution:
    def findItinerary(self, tickets: List[List[str]]) -> List[str]:
        ...
        for start, dest in tickets:
            graph[start] = graph.get(start, []) + [dest]
            graph[start].sort()

        def build_sequence(dest):
            while (dest in graph) and (len(graph[dest]) > 0):
                build_sequence(graph[dest].pop(0))
