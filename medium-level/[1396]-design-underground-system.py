...
# --------------- Runtime 258 ms, beats 28.23%. Memory 27.8MB, beats 9.31% ---------------
...
class UndergroundSystem:
    ...
    def checkOut(self, id: int, stationName: str, t: int) -> None:
        start_station, time = self.checks[id]
        stations = (start_station, stationName)

        self.total_time[stations] = self.total_time.get(stations, [0, 0])
        self.total_time[stations][0] += t - time
        self.total_time[stations][1] += 1

    def getAverageTime(self, startStation: str, endStation: str) -> float:
        time, trips = self.total_time[(startStation, endStation)]
        return time / trips
